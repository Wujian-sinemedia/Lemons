...
def remove_lemons_namespaces(source):
    """Return a string of source code with any lemons namespaces removed.
    """
    namespace_regex = re.compile(r"\s+namespace\s+lemons\s*{")
...
def add_doxygen_group(path, group_name):
    """Add a Doxygen group to the file at 'path'.
       ...
       The addition of lemons namespacing code to all of the source files breaks
    ...
    if re.match(r"^lemons_.*\.(h|dox)", filename):
        ...
        with open(path, "w") as f:
            ...
            f.write(remove_lemons_namespaces(content))
            f.write("\r\n/** @}*/\r\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--subdirs",
                        help="if specified, only include these comma separated subdirectories")
