...
function (lemons_set_aax_signing_settings)
	...
	_lemons_aax_sign_config_check_for_option (
		ACCOUNT "Account ID for AAX plugin signing")
	_lemons_aax_sign_config_check_for_option (SIGNID
											  "SignID for AAX signing on Mac")
	_lemons_aax_sign_config_check_for_option (
		KEYFILE "Keyfile for AAX signing on Windows")
	_lemons_aax_sign_config_check_for_option (
		KEYPASSWORD "Keypassword for AAX signing on Windows")
...
function (lemons_configure_aax_plugin_signing)
	...
	if (APPLE)
		...
		add_custom_command (
			...
			COMMAND
				"${WRAPTOOL_PROGRAM}" ARGS sign --verbose --dsig1-compat off
				--account "${LEMONS_AAX_ACCOUNT}" --wcguid "${LEMONS_AAX_GUID}"
				--signid "${LEMONS_AAX_SIGNID}" --in
	...
	elseif (WIN32)
		...
		add_custom_command (
			...
			COMMAND
				"${WRAPTOOL_PROGRAM}" ARGS sign --verbose --dsig1-compat off
				--account "${LEMONS_AAX_ACCOUNT}" --keyfile
				"${LEMONS_AAX_KEYFILE}" --keypassword
