...
void inverseCepstral (const double* BV_R_ magIn, double* BV_R_ cepOut) override
{
    ...
    vDSP_vsaddD (m_dspare, vDSP_Stride(1), &tinyvalue, m_dspare, vDSP_Stride(1), vDSP_Length(hs1));
...
BV_FORCE_INLINE void packComplex (const double* const re, const double* const im)
{
    ...
    if (re == nullptr)
        vDSP_vfillD (&zero, m_dpacked->realp, vDSP_Stride(1), vDSP_Length(num));
    else
        memcpy (m_dpacked->realp, re, (size_t) num * sizeof (double));
    
    if (im == nullptr)
        vDSP_vfillD (&zero, m_dpacked->imagp, vDSP_Stride(1), vDSP_Length(num));
    else
