...
struct NonParamValueTreeNodeGroup
{
    ...
    private:
        NonParamValueTreeNodeGroupNode (
            std::unique_ptr< NonParamValueTreeNodeGroup > group_)
            : group (std::move (group_))
        {
        }

        NonParamValueTreeNodeGroupNode (
            std::unique_ptr< NonParamValueTreeNode > node_)
            : node (std::move (node_))
        {
        }
    ...
    NonParamValueTreeNodeGroup (const juce::String&               groupName,
                                std::unique_ptr< NodeOrSubgroup > firstChild,
    ...
    {
        return const_cast< const NonParamValueTreeNodeGroupNode** > (
            children.begin());
    }
    ...
    template < typename NodeOrSubgroup, typename... Args >
    void addChild (std::unique_ptr< NodeOrSubgroup > firstChild,
                   Args&&... remainingChildren)
    {
