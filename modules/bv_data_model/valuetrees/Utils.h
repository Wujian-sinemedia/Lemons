...
static inline void
    createValueTreeFromNonParamNodes (juce::ValueTree&                  tree,
                                      const NonParamValueTreeNodeGroup& propertyTree)
{
    ...
    for (auto* node : propertyTree)
    {
        if (auto* thisNode = node->getNode())
        {
            ...
            jassert (child.isValid());
            child.setProperty (
                DefaultValueTreeIds::NonParameterName, thisNode->longName, nullptr);
            tree.addChild (child, -1, nullptr);
        }
...
static inline juce::ValueTree
    getChildTreeForNonParamProperty (juce::ValueTree&                   topLevelTree,
                                     const NonParamValueTreeNode* const property)
{
    jassert (topLevelTree.isValid() && property != nullptr);

    auto child = topLevelTree.getChildWithProperty (
        DefaultValueTreeIds::NonParameterName, property->longName);
