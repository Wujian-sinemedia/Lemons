...
public:
    Spline (const juce::Array< juce::Point< double > >& points)
    {
        ...
        for (int i = 1; i < n; i++)
        {
            ...
            l.set (i,
                   (2 * (points[i + 1].getX() - points[i - 1].getX()))
                       - (h[i - 1]) * u[i - 1]);
            u.set (i, l[i] == 0 ? 0 : h[i] / l[i]);
            a.set (i,
                   h[i] == 0 || h[i - 1] == 0
                       ? 0
                       : (3.0 / (h[i])) * (points[i + 1].getY() - points[i].getY())
                             - (3.0 / (h[i - 1]))
        ...
        for (int j = n - 1; j >= 0; j--)
        {
            ...
            b.set (j,
                   h[j] == 0 ? 0
                             : (points[j + 1].getY() - points[j].getY()) / (h[j])
        ...
        for (int i = 0; i < n; i++)
            elements.add (
                Element (points[i].getX(), points[i].getY(), b[i], c[i], d[i]));
    }
