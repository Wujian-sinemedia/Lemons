                                               ...
                                               const bool   isBypassed)
{
    ...
    {
        processWrapped (input,
                        output,
                        midiMessages,
                        applyFadeIn,
                        applyFadeOut,
                        processingBypassedThisFrame);
        return;
    }
    ...
    do
    {
        ...
        AudioBuffer inputProxy (input.getArrayOfWritePointers(),
                                numInChannels,
                                startSample,
                                chunkNumSamples);
        ...
        bav::midi::copyRangeOfMidiBuffer (
            midiMessages, midiChoppingBuffer, startSample, 0, chunkNumSamples);

        processWrapped (inputProxy,
                        outputProxy,
                        midiChoppingBuffer,
                        applyFadeIn,
                        applyFadeOut,
        ...
        bav::midi::copyRangeOfMidiBuffer (
            midiChoppingBuffer, midiMessages, 0, startSample, chunkNumSamples);
...
template < typename SampleType >
void FIFOWrappedEngine< SampleType >::processWrapped (AudioBuffer& input,
                                                      AudioBuffer& output,
                                                      MidiBuffer&  midiMessages,
                                                      ...
                                                      const bool   isBypassed)
{
    ...
    if (applyFadeIn) output.applyGainRamp (0, numNewSamples, 0.0f, 1.0f);

    if (applyFadeOut) output.applyGainRamp (0, numNewSamples, 1.0f, 0.0f);
}
...
template < typename SampleType >
void FIFOWrappedEngine< SampleType >::initialize (double samplerate,
                                                  int    newInternalBlocksize)
{
    jassert (samplerate > 0);
    jassert (newInternalBlocksize > 0);
