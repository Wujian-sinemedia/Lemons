    ...
    double frequency, double sampleRate) const noexcept
{
    ...
    std::complex< double > jw =
        std::exp (-juce::MathConstants< double >::twoPi * frequency * j / sampleRate);

    for (int n = 0; n <= order; ++n)
    {
        numerator += static_cast< double > (coefficients[n]) * factor;
        factor *= jw;
    }
    ...
    {
        denominator += static_cast< double > (coefficients[n]) * factor;
        factor *= jw;
    }
    ...
    double        sampleRate) const noexcept
{
    ...
    for (size_t i = 0; i < numSamples; ++i)
    {
        ...
        std::complex< double > numerator = 0.0, denominator = 0.0, factor = 1.0;
        std::complex< double > jw = std::exp (-juce::MathConstants< double >::twoPi * frequencies[i]
                                              * j / sampleRate);
...
template < typename NumericType >
double
    Coefficients< NumericType >::getPhaseForFrequency (double frequency,
                                                       double sampleRate) const noexcept
{
    constexpr std::complex< double > j (0, 1);
    const auto                       order = getFilterOrder();
