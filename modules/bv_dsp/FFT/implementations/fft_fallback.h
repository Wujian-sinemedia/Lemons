...
public:
                             ...
                             float* BV_R_       complexOut) override
    {
        ...
        for (int i = 0; i <= m_half; ++i)
            complexOut[i * 2] = float (m_c[i]);
        for (int i = 0; i <= m_half; ++i)
            complexOut[i * 2 + 1] = float (m_d[i]);
    }
...
private:
    ...
    BV_FORCE_INLINE void
        transformF (const T* BV_R_ ri, double* BV_R_ ro, double* BV_R_ io)
    {
    ...
    template < typename T >
    BV_FORCE_INLINE void
        transformI (const double* BV_R_ ri, const double* BV_R_ ii, T* BV_R_ ro)
    {
        int halfhalf = m_half / 2;
    ...
    BV_FORCE_INLINE void transformComplex (const double* BV_R_ ri,
                                           const double* BV_R_ ii,
                                           double* BV_R_       ro,
                                           double* BV_R_       io,
                                           bool                inverse)
    {
        ...
        for (int blockSize = 2; blockSize <= n; blockSize <<= 1)
        {
            ...
            {
                double phase =
                    2.0 * juce::MathConstants< double >::pi / double (blockSize);
