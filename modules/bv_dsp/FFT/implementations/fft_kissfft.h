...
namespace bav::dsp
{
    ...
    public:
        ...
        void forwardInterleaved(const float *BQ_R__ realIn, float *BQ_R__ complexOut) override
        {
            kiss_fftr (m_fplanf, realIn, (kiss_fft_cpx *)complexOut);
        }
        ...
        void forwardPolar (const double* BV_R_ realIn, double* BV_R_ magOut, double* BV_R_ phaseOut) override
        {
            vecops::convert (m_fbuf, realIn, m_size);
            kiss_fftr (m_fplanf, m_fbuf, m_fpacked);
            vecops::cartesian_interleaved_to_polar (magOut, phaseOut, m_fpacked, m_size/2+1);
        }
        ...
        void forwardMagnitude (const double* BV_R_ realIn, double* BV_R_ magOut) override
        {
            vecops::convert (m_fbuf, realIn, m_size);
            kiss_fftr (m_fplanf, m_fbuf, m_fpacked);
            vecops::cartesian_interleaved_to_magnitudes (magOut, (float *)m_fpacked, m_size/2+1);
        }
        ...
        */
        ...
        void inverse (const double* BV_R_ realIn, const double* BV_R_ imagIn, double* BV_R_ realOut)
        {
        ...
        {
            vecops::polar_to_cartesian_interleaved (m_fpacked, magIn, phaseIn, m_size/2+1);
            kiss_fftri (m_fplani, m_fpacked, m_fbuf);
            vecops::convert (realOut, m_fbuf, m_size);
        }
