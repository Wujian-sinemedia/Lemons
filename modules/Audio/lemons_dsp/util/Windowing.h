...
template <typename ValueType>
static const WindowingFunction<ValueType> rectangular = [] (int, int)
{ return ValueType (1); };

template <typename ValueType>
static const WindowingFunction<ValueType> triangular = [] (int size, int i)
{
...
template <typename ValueType>
inline ValueType ncos (int order, int i, int size) noexcept
{
	return std::cos (static_cast<ValueType> (order * i)
					 * juce::MathConstants<ValueType>::pi / static_cast<ValueType> (size - 1));
}

}  // namespace detail

template <typename ValueType>
static const WindowingFunction<ValueType> hann = [] (int size, int i)
{
	return static_cast<ValueType> (0.5 - 0.5 * detail::ncos<ValueType> (2, i, size));
};

template <typename ValueType>
static const WindowingFunction<ValueType> hamming = [] (int size, int i)
{
	return static_cast<ValueType> (0.54 - 0.46 * detail::ncos<ValueType> (2, i, size));
};

template <typename ValueType>
static const WindowingFunction<ValueType> blackman = [] (int size, int i)
{
...
template <typename ValueType>
static const WindowingFunction<ValueType> blackmanHarris = [] (int size, int i)
{
	const auto cos2 = detail::ncos<ValueType> (2, i, size);
	const auto cos4 = detail::ncos<ValueType> (4, i, size);
...
template <typename ValueType>
static const WindowingFunction<ValueType> flatTop = [] (int size, int i)
{
	const auto cos2 = detail::ncos<ValueType> (2, i, size);
	const auto cos4 = detail::ncos<ValueType> (4, i, size);
	const auto cos6 = detail::ncos<ValueType> (6, i, size);
