...
public:
    ...
    ~AsyncDownload() override { stopThread (100); }
    ...
    bool readEntireBinaryStream (juce::MemoryBlock& destData,
                                 bool               usePostCommand = false)
    {
        const std::unique_ptr< juce::InputStream > input (
            url.isLocalFile() ? url.getLocalFile().createInputStream()
                              : url.createInputStream (
                                  usePostCommand, nullptr, nullptr, {}, timeoutMS));
