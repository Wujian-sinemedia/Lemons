...
static inline juce::AudioProcessorParameterGroup*
    findParameterSubgroup (const juce::AudioProcessorParameterGroup* parameterTree,
                           const juce::String&                       subgroupName)
{
    ...
    for (auto* node : *parameterTree)
        if (auto* group = node->getGroup())
            if (group->getName() == subgroupName) return group;
...
static inline void parseParameterTreeForParameterPointers (
